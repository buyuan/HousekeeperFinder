...
export class RiderDashboardComponent implements OnInit, OnDestroy {
  ...
  updateToast(trip: Trip): void {
    if (trip.status === 'STARTED') {
      this.toastr.info(`Housekeeper ${trip.driver!.username} has taken the order.`);
    } else if (trip.status === 'IN_PROGRESS') {
      this.toastr.info(`Housekeeper ${trip.driver!.username} is doing housekeeping.`);
    } else if (trip.status === 'COMPLETED') {
      this.toastr.info(`Housekeeper ${trip.driver!.username} has finished the job.`);
    }
  }
